	...
	for row in reversed(data):
		if row.indent == 0.0:
			if count:
				...
				row.purchase_rate = flt(purchase_rate / count, float_precision)
				row.profit_percentage = flt(profit_percentage / count, float_precision)
			else:
				row.sales_rate = 0
				row.purchase_rate = 0
				row.profit_percentage = 0
...
class GrossProfitGenerator:
	...
	def get_buying_amount_from_so_dn(self, sales_order, so_detail, item_code):
		...
		query = (
			...
			.where(delivery_note_item.docstatus != 2)
	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql(
			...
			from
				...
				{payment_term_table}
			where
				`tabSales Invoice`.docstatus!=2 and `tabSales Invoice`.is_opening!='Yes' {conditions} {match_cond}
			order by
	...
	def get_delivery_notes(self):
		...
		if self.si_list:
			...
			delivery_notes = (
				...
				.where((dni.docstatus != 2) & (dni.against_sales_invoice.isin(invoices)))
